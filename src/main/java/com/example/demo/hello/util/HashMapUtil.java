...
public class HashMapUtil<K,V> {
    ...
    //构造函数，指定初始化容量和初始化加载因子
    public HashMapUtil(int initialCapacity,float loadFactor) {
    ...
    final Node<K,V>[] resize(){
        Node<K,V>[] oldTab = table; //旧table指向原来的table
        int oldCap = (oldTab == null)? 0 : oldTab.length;
        int oldThr = threshold;
        int newCap = 0;
        int newThr = 0;
        if(oldCap > 0){ //说明将要进行扩容操作
            if(oldCap >= MAXIMUM_CAPACITY){ //由于最大容量不超过MAXIMUM_CAPACITY ，当原来的容量达到这个值后不能在进行扩容
                threshold = Integer.MAX_VALUE;
                return oldTab;
            }else if((newCap = oldCap << 1) < MAXIMUM_CAPACITY && oldCap >= DEFAULT_INITIAL_CAPACITY) { //进行两倍扩容
                newThr = oldThr << 1;
            }
        }else if(oldThr > 0){
            newCap = oldThr;
    ...
    //构造函数，指定初始容量，默认加载因子
    public HashMapUtil(int initialCapacity){
    ...
    static final class TreeNode<K,V> extends LinkedHashMapUtil.Entry<K,V> {
         ...
         * this node.
         */
        final Node<K,V> untreeify(HashMapUtil<K,V> map) {
         ...
         */
        final TreeNode<K,V> putTreeVal(HashMapUtil<K,V> map, Node<K,V>[] tab,
                                               int h, K k, V v) {
            Class<?> kc = null;
         ...
         */
        final void removeTreeNode(HashMapUtil<K,V> map, Node<K,V>[] tab,
                                  boolean movable) {
            int n;
         ...
         */
        final void split(HashMapUtil<K,V> map, Node<K,V>[] tab, int index, int bit) {
            TreeNode<K,V> b = this;
